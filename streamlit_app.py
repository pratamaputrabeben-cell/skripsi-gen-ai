...
# --- 2. FUNGSI MEMBERSIHKAN TEKS DARI AI ---
def bersihkan_teks_ai(teks):
    # Hapus basa-basi AI di awal (Tentu, Berikut adalah..., dsb)
    teks = re.sub(r"^(Tentu|Berikut|Ini adalah|Sesuai).*?\n", "", teks, flags=re.IGNORECASE)
    # Hapus tag HTML seperti &nbsp;
    teks = teks.replace("&nbsp;", " ")
    # Hapus garis pembatas markdown ---
    teks = teks.replace("---", "")
    # Hapus tanda bintang berlebihan untuk bold markdown
    teks = teks.replace("**", "")
    return teks.strip()

# --- 3. FUNGSI RAPIKAN WORD (4-3-3-3) ---
def buat_dokumen_rapi(judul_bab, isi_teks):
    ...
    isi_clean = bersihkan_teks_ai(isi_teks)
    paragraphs = isi_clean.split('\n')
    ...
    for p_text in paragraphs:
        ...
        if t:
            ...
            # Deteksi Sub-bab (1.1, 1.2, dst)
            match_sub = re.match(r"^(\d+\.\d+(\.\d+)?)\s*(.*)", t)
            if match_sub:
                p.clear() # Bersihkan paragraf untuk formatting ulang
                run = p.add_run(match_sub.group(0))
                run.bold = True
                # Atur indentasi berdasarkan level (1.1 = 0.2, 1.1.1 = 0.4)
                level = match_sub.group(1).count('.')
...
# --- 4. UI & PROSES ---
st.set_page_config(page_title="SkripsiGen Pro v8.49", layout="wide")

with st.sidebar:
    st.header("🛡️ Pusat Kalibrasi")
    st.success("✅ Clean Formatting System")
...
st.title("🎓 SkripsiGen Pro v8.49")
st.caption("Standard: Clean Academic Output | No HTML Tags | Margin 4333")
...
def jalankan_proses(target_bab=None, catatan_dosen=""):
    ...
    if topik and nama_user:
        with st.spinner(f"Menyusun & Membersihkan {bab_aktif}..."):
            try:
                model = inisialisasi_ai()
                prompt = f"Susun draf {bab_aktif} skripsi {metode} judul '{topik}' di {lokasi}, {kota}. Revisi: {catatan_dosen}. Anti-Plagiarism, Paraphrase, Ref RIIL 2023-2026."
                res = model.generate_content(prompt)
                st.session_state['db'][bab_aktif] = res.text
                st.success(f"{bab_aktif} Siap!")
        ...
        with st.container(border=True):
            ...
            with st.expander(f"Buka Draf {b}"):
                ...
                if is_trial or is_pro:
                    ...
                    st.error(f"🔑 Terkunci (PRO)")
                    st.link_button("💬 Hubungi Admin", f"https://wa.me/6281273347072?text=Mau%20Lisensi%20PRO%20{nama_user}")
